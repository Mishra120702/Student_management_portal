...
// Define reCAPTCHA keys (use different keys than student login if needed)
...
// Redirect if already logged in as trainer
if (isset($_SESSION['user_id']) && $_SESSION['user_role'] === 'mentor') {
    header("Location: dash_t/dashboard.php");
    exit;
}
...
if (isset($_POST['login'])) {
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        if (!isset($_POST['g-recaptcha-response'])) {
            ...
            // Verify with Google using cURL instead of file_get_contents
            ...
            if (curl_errno($ch)) {
                ...
                $login_error = 'Unable to verify reCAPTCHA. Please try again later.';
            } else {
                $response_data = json_decode($response);
                
                if (!$response_data || !isset($response_data->success)) {
                    error_log("Invalid reCAPTCHA response format");
                    $login_error = 'Invalid reCAPTCHA verification. Please try again.';
                    ...
                    error_log("reCAPTCHA failed for IP: " . $_SERVER['REMOTE_ADDR'] . " Errors: " . $error_codes);
                } else {
                    ...
                    if (empty($username) || empty($password)) {
                        ...
                        if ($attempts > 10) {
                            ...
                            $stmt = $db->prepare("SELECT * FROM users WHERE name = ? AND role = 'mentor'");
...
<body>
    <div class="login-container">
        <div class="brand">
            <h2>ASD Academy Trainer Portal</h2>
            <p>Trainer Sign In</p>
        </div>
        <form action="log2.php" method="post" autocomplete="off">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'], ENT_QUOTES, 'UTF-8'); ?>">
            
            <div class="form-group">
                <label for="name">Trainer Name</label>
                <input type="text" id="name" name="name" required placeholder="Enter trainer name" autocomplete="username">
            </div>
